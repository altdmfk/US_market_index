...
- **복합키 멱등 저장**: `date + data_type` 형태의 고유 복합키(예: `2026-08-24_fear_and_greed`)를 사용하여 같은 날짜에 여러 번 호출해도 중복 생성 없이 원자적으로 갱신합니다.
- **과거 이력 보존**: 일별 스냅샷 테이블을 통해 최소 2개 이상의 서로 다른 날짜 기록을 관리합니다.
