...
- **직접 판단한 일**: AI가 S&P 500 전일 종가(`previousClose`)를 125일 이평선이나 같은 날 마커로 잘못 참조하던 오류를 찾아 바로잡음; Supabase 클라우드 DB를 연동하여 이력 공유 구축; 타임스탬프와 차트 X축을 한국 표준시(`Asia/Seoul`, KST)로 통일. 대시보드 뷰에서는 최신 10건만 조회하도록 최적화; 비밀번호 노출 방지를 위해 SHA-256 해시 검증 도입.
- **AI 제안을 따르지 않은 일 (없으면 이유)**: 과거 데이터 생성을 위해 코드에 날짜/기본값을 하드코딩하거나 임의 Backfilling으로 차트를 그리려던 제안을 반려하고 DB에 적재된 스냅샷만 렌더링하도록 강제함; 복잡한 비정상 복구 버튼을 5대 장애 모드와 직관적 1-Click 재시도로 단순화함
