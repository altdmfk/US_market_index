...
- **3단계 이내 행동**: 메인 화면에서 실시간 지표를 확인한 뒤, 상단 우측 자물쇠 아이콘을 클릭하여 비밀번호 입력 후 개발자 모드에 진입하여 Error simulation 장애 버튼 및 Test tools를 조작한다.
...
- **AI에게 맡긴 일**: CNN Fear & Greed 및 Yahoo Finance 공개 API 연동과 프록시 폴백 파이프라인 구성, 5대 장애 모드(Timeout, 401/403, 429, Offline, Schema break) FSM 상태 머신 설계, Supabase PostgreSQL 클라우드 DB 연동 및 GitHub Actions 일별 Cron 스크립트 작성, Tailwind CSS 반응형 대시보드 및 차트 컴포넌트 코드 구현.
- **직접 판단한 일**: AI가 S&P 500 전일 종가(`previousClose`)를 125일 이평선이나 같은 날 마커로 잘못 참조하던 오류를 지적하여 실제 직전 거래일 종가(`7,641.16 USD`, QQQ `710.93 USD`)로 바로잡음; 브라우저 로컬 저장소의 기기 간 격차 한계를 파악하고 Supabase 클라우드 DB를 연동하여 영구 이력 공유 구축; 모든 타임스탬프와 차트 X축을 한국 표준시(`Asia/Seoul`, KST)로 통일하고 이력 보존을 최대 10건으로 제한함; 비밀번호 노출 방지를 위해 SHA-256 해시 검증 도입.
- **AI 제안을 따르지 않은 일 (없으면 이유)**: AI가 과거 데이터 생성을 위해 코드 내에 날짜나 임시 기본값을 하드코딩하려던 제안을 단호히 반려하고 순수 동적 API 타임스탬프 파싱 및 Supabase DB를 단일 진실 공급원으로 사용하도록 강제함; 선 그래프 마우스 호버 시 점이 흔들리던 `animate-ping` 이펙트를 기각하고 정적 고정 렌더링으로 수정함; Supabase upsert 시 발생한 409 Conflict 오류를 지적하여 `on_conflict` 파라미터로 즉시 해결하도록 지시함.
...
### 6.1 접근 및 보안 검증
- **진입 경로**: 상단 헤더 우측의 자물쇠(Lock) 아이콘 클릭 후 비밀번호 입력.
